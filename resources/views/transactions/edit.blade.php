<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="overflow-hidden rounded-lg bg-white p-6 shadow-sm transition-colors duration-200">
                <h3 class="mb-5 text-lg font-semibold text-gray-800">Editar Transação</h3>
                ...
                <form method="POST" action="{{ route('transactions.update', $transaction) }}" class="space-y-5">
                    ...
                    <div>
                        ...
                        <select id="type" name="type" @class([
                            'mt-1 block w-full rounded-md border bg-white px-3 py-2 text-sm text-gray-900 shadow-sm focus:outline-none focus:ring-2',
                    ...
                    <div>
                        ...
                        <input id="amount" name="amount" type="number" step="0.01" min="0.01" value="{{ old('amount', $transaction->amount) }}" @class([
                            'mt-1 block w-full rounded-md border bg-white px-3 py-2 text-sm text-gray-900 shadow-sm focus:outline-none focus:ring-2 placeholder:text-gray-400',
                    ...
                    <div>
                        ...
                        <input id="date" name="date" type="date" value="{{ old('date', optional($transaction->date)->toDateString() ?? now()->toDateString()) }}" @class([
                            'mt-1 block w-full rounded-md border bg-white px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 text-gray-900',
                    ...
                    <div class="sticky bottom-0 mt-8 border-t border-gray-200 bg-white/95 pt-4 backdrop-blur transition-colors duration-200">
                        <div class="flex items-center justify-end gap-3">
                            <a href="{{ route('transactions.index') }}" class="inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 transition hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-200">
                            Cancelar
                            </a>
